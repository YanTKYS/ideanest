...
| M11 | Unit Testプロジェクト追加 | `IdeaNest.Tests` を追加し、Service / ViewModel 周辺の単体テストを整備する。MainViewModel分割前の安全網として優先度が高い | A |
| M12 | MainViewModelの段階的分割 | `MainViewModel` の責務が肥大化しているため、タグ管理・フィルタ・エクスポートなど影響範囲の小さい領域からサブViewModelへ分割する | B |

...
| M12 | MainViewModelの段階的分割 (継続中) | v0.8.1 で `CardDisplayViewModel` を抽出済み。次の候補: `FilterViewModel` (SearchText / SelectedTag / SelectedColor / ShowArchived)、`TagPanelViewModel` (IsTagPanelOpen 等)、`TagManagementViewModel` (RenameTag / DeleteTag)。詳細方針は `design-decisions.md` 参照 | B |

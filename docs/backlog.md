...
| M12 | MainViewModelの段階的分割 | `MainViewModel` の責務が肥大化しているため、タグ管理・フィルタ・エクスポートなど影響範囲の小さい領域からサブViewModelへ分割する。v0.8.0 で追加した `tests/IdeaNest.Tests` を安全網として利用 / 拡張しながら進める | B |

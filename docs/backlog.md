...
IdeaNest と NoteNest のツール間連携に関する設計・ドキュメント整備の項目。
詳細は `docs/integration-notenest.md` を参照。

| No | 項目 | 概要 | 優先度 |
| --- | --- | --- | --- |
| I1 | NoteNest 向けエクスポートの改善 | 現在の `[NOTE]`/`[TODO]` マーカー記法を NoteNest 側の「公式インポートフォーマット」として合意し、仕様を明文化する。IdeaNest 単体ではなく NoteNest 側との協議が必要 | B |
| I2 | エクスポートマーカー文言のカスタマイズ | NoteNest向けエクスポートの `[NOTE]` / `[TODO]` 文言を自由入力で変更できるようにする (backlog M10 と同一。こちらを正とする) | C |

### Nest 共通化検討

IdeaNest / NoteNest で共通化できる処理やデータ仕様の検討項目。
現時点では共通ライブラリ化よりも「テキスト形式を共通インターフェースとする」方針を優先する。

| No | 項目 | 概要 | 優先度 |
| --- | --- | --- | --- |
| N1 | タグ正規化仕様の共通化 | `docs/integration-notenest.md` に記載のとおり、ロジックは同じだが共通ライブラリ化のコストが高い。まず仕様ドキュメントを揃えるところから始める | C |
| N2 | JSON スキーマバージョン管理の導入 | `.ideanest` に `schemaVersion` フィールドを追加し、旧バージョン読み込み時にマイグレーション処理を走らせる (backlog M6 と同一。こちらを正とする) | B |

### NestSuite / ランチャー

IdeaNest と NoteNest を統合するランチャーやハブアプリに関する長期検討項目。
現時点では両ツールの単体完成度を優先し、実装しない。詳細は `docs/integration-notenest.md` を参照。

| No | 項目 | 概要 | 優先度 |
| --- | --- | --- | --- |
| S1 | NestSuite ランチャー | IdeaNest / NoteNest を同じランチャーから起動し、最近使ったファイルを統合表示するハブアプリ。アプリ間の設定共有や共通リリースサイクルなど検討事項が多い | C |
| S2 | 共通カラーパレット | IdeaNest / NoteNest でカードや表示要素の色定義を揃える。現在は各アプリに実装。H2 (ユーザー定義カラーパレット) と合わせて検討 | C |

...
| No | 項目 | 概要 | 優先度 |
| --- | --- | --- | --- |
| ~~L1~~ | ~~プレビューの前後カード移動~~ | ~~詳細プレビューウィンドウに「← 前へ」「次へ →」ボタン (または `←` / `→` キー) を追加し、同一フィルタ内のカードを連続して閲覧できるようにする~~ | ~~A~~ → **v0.6.2 で実装済み** |
...
| ~~L5~~ | ~~最近使ったファイルのクリア~~ | ~~スタートダイアログまたは設定から「最近使ったファイルをすべてクリア」できるようにする。共有端末や検証時に不要な履歴を残したくない場合に有用~~ | ~~B~~ → **v0.6.4 で実装済み** |
| ~~L6~~ | ~~「壁打ち」1 枚ランダム表示~~ | ~~「ランダムに 1 枚選んでプレビューで表示する」コマンドを追加。既存のシャッフルロジックを流用でき、追加コストは小さい~~ | ~~B~~ → **v0.6.4 で実装済み** |

...
| M12 | MainViewModelの段階的分割 (継続中) | v0.8.1〜v0.8.8 で `CardDisplayViewModel` / `FilterViewModel` / `TagPanelViewModel` / `ExportViewModel+IExportPlatform` / `RecentFilesService+StartupCoordinator+StartupViewModel` / `SaveStateViewModel` / `CardOperationsService+TagSyncService` を抽出済み。次の候補: `TagManagementViewModel` (RenameTag / DeleteTag / AvailableTags、AllCards 参照あり要設計)。詳細方針は `design-decisions.md` 参照 | B |

...
| L5 | 最近使ったファイルのクリア | スタートダイアログまたは設定から「最近使ったファイルをすべてクリア」できるようにする。共有端末や検証時に不要な履歴を残したくない場合に有用 | B |
| L6 | 「壁打ち」1 枚ランダム表示 | 「ランダムに 1 枚選んでプレビューで表示する」コマンドを追加。既存のシャッフルロジックを流用でき、追加コストは小さい | B |

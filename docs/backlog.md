...
| M12 | MainViewModelの段階的分割 (継続中) | v0.8.1〜v0.8.9 で `CardDisplayViewModel` / `FilterViewModel` / `TagPanelViewModel` / `ExportViewModel+IExportPlatform` / `RecentFilesService+StartupCoordinator+StartupViewModel` / `SaveStateViewModel` / `CardOperationsService+TagSyncService` / `TagManagementService` を抽出済み。WPF 非依存の主要な純粋ロジックは出尽くしたため、今後は `RefreshVisible` の絞り込み・ソート段や自動保存スケジューラ周辺を必要に応じて検討する。詳細方針は `design-decisions.md` 参照 | B |
